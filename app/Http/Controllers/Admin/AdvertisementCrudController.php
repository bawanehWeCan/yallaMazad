...
//         if ($ad->status=='approve'||$ad->status=='rejected') {
        ...
        if ($this->data['entry']->status=='approve') {
            return $this->adNotificationSend($this->data['entry']->id,$this->data['entry']->status,'Advertisement Approval','Advertisement has been approved',$this->data['entry']->user->device_token);
